...
ASSISTANT_INSTRUCTIONS = (
	...
	"- read_file(file): the text of an attachment or public link — pdf, docx, xlsx, html, plain text, "
	"images and scans (OCR'd). Pass a File name, an Attach field's path, or a URL. Never judge a "
	"document's contents without reading it.\n"
	...
	"read_file, create, update, delete, run_action) already cover all standard Frappe work: reading, "
	"writing, submitting, workflows, custom fields, reading attached documents, multi-record ops. An "
	"agent that must work from resumes, invoices, contracts or any other attachment needs read_file — "
	"never write a Flow Tool to parse files. When you create a Flow Agent, assign it "
